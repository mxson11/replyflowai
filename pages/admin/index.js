                ...
                <div key={b.id} style={{ border: selectedBiz?.id === b.id ? '1px solid #00D97E' : '1px solid #26262A', borderRadius: 10, padding: 14, background: '#141416' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button onClick={() => {
                        const link = `${window.location.origin}/dashboard/${b.access_code}`;
                        navigator.clipboard.writeText(link);
                        alert('Client dashboard link copied:\n' + link);
                      }} style={{ fontSize: 11.5, padding: '6px 10px', borderRadius: 6, background: 'transparent', border: '1px solid #26262A', color: '#F5F5F3', cursor: 'pointer' }}>Copy client link</button>
